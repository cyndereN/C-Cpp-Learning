...
HitInfo intersectPlane(const Ray ray,const Plane plane, const float tMin, const float tMax) {
	...
	// Plane equation: (p-p')*N=0, p is a point on plane, p' is the intersection
	vec3 p = plane.normal * plane.d;
	float t = (dot(p, plane.normal) - dot(ray.origin, plane.normal)) / dN;
	
	vec3 normal = ray.origin.y > p.y ? plane.normal : -plane.normal ;
...
HitInfo intersectCylinder(const Ray ray, const Cylinder cylinder, const float tMin, const float tMax) {
	...
	float dotRayCylinder = dot(ray.direction, cylinder.direction);
	vec3 q = ray.direction - (dotRayCylinder * cylinder.direction);
	float a = dot(q, q);
	float b = 2.0 * dot(q, p - dot(p, cylinder.direction) * cylinder.direction);
	
	vec3 r = p - dot(p, cylinder.direction) * cylinder.direction;
	float c = dot(r, r) - pow(cylinder.radius, 2.0);
...
HitInfo intersectBoolean(const Ray ray, const Boolean boolean, const float tMin, const float tMax) {
	...
	Sphere sphere_1 = boolean.spheres[0];
	Sphere sphere_2 = boolean.spheres[1];

	HitInfo hitInfo_1 = intersectSphere(ray, sphere_1, tMin, tMax);
	HitInfo hitInfo_2 = intersectSphere(ray, sphere_2, tMin, tMax);

	if(boolean.mode == BOOLEAN_MODE_AND) {
		if (inside(hitInfo_2.position, sphere_1) )
			return hitInfo_2;
		if (inside(hitInfo_1.position, sphere_2) )
			return hitInfo_1;
	...
	if (boolean.mode == BOOLEAN_MODE_MINUS) {
		...
		float tMin_prime = hitInfo_1.t + 0.0001;
		HitInfo hitInfo_1 = intersectSphere(ray, sphere_1, tMin_prime, tMax);
		if (inside(hitInfo_1.position, sphere_2))
			return HitInfo(
					hitInfo_1.hit,
					hitInfo_1.t,
					...
					-hitInfo_1.normal,
					hitInfo_1.material,
					hitInfo_1.enteringPrimitive
				);
		
	}
	
	// I would change the member "t" of HitInfo to "t0" and "t1" because both are needed for calculating the result
...
float fresnel(const vec3 viewDirection, const vec3 normal, const float sourceIOR, const float destIOR) {
	...
	// Reflectance increases with gazing angle
	// ref: https://en.wikipedia.org/wiki/Fresnel_equations
	// Schlick Approximation
	// if theta = 90°, the reflectance is 1
	// if theta = 0°, the reflectance is r_0, which is resonable
	float theta = acos(dot(-viewDirection, normal));
	float r_0 = pow(((destIOR - sourceIOR ) / (sourceIOR + destIOR)), 2.0);
	float reflectance = r_0 + (1.0 - r_0) * pow((1.0 - cos(theta)), 5.0);
	return reflectance;
#else
...
vec3 colorForFragment(const Scene scene, const vec2 fragCoord) {
...
#ifdef SOLUTION_FRESNEL
		float reflectionRate = fresnel(currentRay.direction, currentHitInfo.normal, currentIOR, currentIOR);
		reflectionWeight *= reflectionRate;
#else
	...
	Ray nextRay;
		...
		vec3 hitToRay = normalize(currentRay.origin - currentHitInfo.position);
		nextRay.direction = -hitToRay + 2.0 * max(0.0, dot(currentHitInfo.normal, hitToRay)) * currentHitInfo.normal;
#else
...
#ifdef SOLUTION_FRESNEL
		float refractionRate = 1.0 - fresnel(currentRay.direction, currentHitInfo.normal, currentIOR, currentHitInfo.material.ior);
		refractionWeight *= refractionRate;
#else
	...
	Ray nextRay;
		...
		float eta_1_2 = sourceIOR / destIOR;
		float alpha = acos(dot(hitToRay, currentHitInfo.normal));
		float root = 1.0 + pow(eta_1_2, 2.0) * (pow(cos(alpha), 2.0) - 1.0);

		bool isTotalInternalReflection = root < 0.0;
		
		if (isTotalInternalReflection)	break;
		
		nextRay.direction = (-eta_1_2 * hitToRay) + (currentHitInfo.normal * (eta_1_2 * cos(alpha) - sqrt(root)));
